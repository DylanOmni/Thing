...
public class OmniBeamEntity extends ThrownItemEntity {
    public OmniBeamEntity(EntityType<? extends ThrownItemEntity> entityType, World world) {
        super(entityType, world);
    }

    public OmniBeamEntity(World world, LivingEntity owner) {
        super(OotW.OmniBeamEntityType, owner, world); // null will be changed later
    }

    public OmniBeamEntity(World world, double x, double y, double z) {
        super(OotW.OmniBeamEntityType, x, y, z, world); // null will be changed later
    }
    ...
    private ParticleEffect getParticleParameters() { // Not entirely sure, but probably has do to with the snowball's particles. (OPTIONAL)
        ...
        return (ParticleEffect)(itemStack.isEmpty() ? ParticleTypes.TOTEM_OF_UNDYING : new ItemStackParticleEffect(ParticleTypes.ITEM, itemStack));
    }
    ...
    protected void onEntityHit(EntityHitResult entityHitResult) { // called on entity hit.
        ...
        if (entity instanceof LivingEntity livingEntity) { // checks if entity is an instance of LivingEntity (meaning it is not a boat or minecart)
            ...
            livingEntity.addStatusEffect((new StatusEffectInstance(StatusEffects.WEAKNESS, 20 * 3, 1))); // applies a status effect
            livingEntity.playSound(SoundEvents.ENTITY_ILLUSIONER_CAST_SPELL, 2F, 1F); // plays a sound for the entity hit only
        }
    }
